...
5. 새 crash나 timeout을 발견하면 검증 전 산출물임을 표시해 Telegram으로 알린다.
   트리아지 후에는 재현 횟수, sanitizer 근거, 오탐 사유와 자동 재개 여부를 포함한 판정을
   별도로 보낸다. 재현된 오류는 검증 에이전트의 신뢰도·영향도, 로컬 PoC와 버그바운티
   보고서 초안 생성 여부까지 후속 알림한다. 같은 파일 내용은 SHA-256으로 중복 알림을
   막는다.
