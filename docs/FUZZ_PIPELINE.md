...
5. **quartet_gate**: P1–P4와 입력 도달 여부를 검사한다. 하네스 자체 오류나 probe의
   ...
   coverage 분석과 장기 실행을 생략하고 즉시 triage한다. 생성 통합의 유일한 하네스가
   실패하면 구체적인 Quartet 근거만 사용해 통합 사본을 제한적으로 자동 수정하고 전체
   빌드와 품질 게이트를 다시 실행한다.
...
최대 10개의 압축된 후보를 로컬 Codex에 한 번 전달한다. 네이티브 생성 프로젝트에 공개
자료가 없는 경우에는 성공한 ASan probe와 Quartet 결과를 로컬 기준선으로 사용한다.
분석이 기존 하네스 실행을
...
전략으로 제한한다. 준비·통합·빌드·smoke·Quartet·coverage·퍼징 단계의 실패는
`recovery_pending`에서 멈춰 동일 명령을 반복하지 않는다. 중앙 AI는 제시된
`retry_stage`, `restart_from_integration`, `skip_target` 중 하나만 선택한다. 통합 재시작은
현재 하네스를 제외하고 이전 산출물을 history에 보관하며, 자동 복구 한도가 끝나면 다음
보상 대상으로 넘어간다. Quartet 후보 및 자동 수정 횟수나 후속 하네스 생성 횟수처럼
이미 정해진 예산을 소진한 경우에는 추가 재시도를 허용하지 않고 `skip_target`만 제시한다.
묶음 종료 후
cycle review와 허용된 후속 하네스 probe가 끝나야 다음 묶음을 시작한다. 문제와 새 crash,
PoC, 검증 보고서는 환경변수로 설정한 Telegram bot에 알린다.
하네스 생성은 작업당 최대 두 사이클로 제한하며, 한도 소진 시 실패 증거를 보존하고
`skipped_after_recovery`로 완료한다.
